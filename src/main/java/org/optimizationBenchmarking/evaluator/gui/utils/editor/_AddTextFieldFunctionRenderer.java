...
final class _AddTextFieldFunctionRenderer extends FunctionRenderer {
  ...
  public final void render(final Page page) throws IOException {
    ...
    out.write(
        ").value;if((name!=null)&&(name.length>0)){var value=document.getElementById(");//$NON-NLS-1$
    page.fieldNameFromPrefixAndNameJS(false, "prefix", false, //$NON-NLS-1$
    ...
    page.fieldNameFromPrefixAndNameJS(true, "prefix", false, //$NON-NLS-1$
        EditorModule.NAME_STRING, false);
    out.write(
        "\\')\"/></td></tr>';var rowToInsertBefore=document.getElementById('");//$NON-NLS-1$
    page.fieldNameFromPrefixAndNameJS(true, "prefix", false, //$NON-NLS-1$
        EditorModule.ADD_FIELD_ROW_ID, true);
    out.write(
        "');var dummy=document.createElement('table');dummy.innerHTML=newbody;var insert=dummy.firstChild;if(insert.tagName.toUpperCase()=='TBODY'){insert=insert.firstChild;}rowToInsertBefore.parentNode.insertBefore(insert,rowToInsertBefore);dummy.innerHTML='");//$NON-NLS-1$
    out.write(EditorModule.CONFIG_ROW_SPACER);
    out.write(
        "';insert=dummy.firstChild;if(insert.tagName.toUpperCase()=='TBODY'){insert=insert.firstChild;}rowToInsertBefore.parentNode.insertBefore(insert,rowToInsertBefore);}}");//$NON-NLS-1$
  }
}
