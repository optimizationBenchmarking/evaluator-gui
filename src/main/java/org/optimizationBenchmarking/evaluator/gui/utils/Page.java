package org.optimizationBenchmarking.evaluator.gui.utils;
...
public final class Page implements Closeable {
  ...
  private static final char[] JAVASCRIPT_START = { '<', 's', 'c', 'r', 'i',
      'p', 't', ' ', 't', 'y', 'p', 'e', '=', '"', 't', 'e', 'x', 't', '/',
      'j', 'a', 'v', 'a', 's', 'c', 'r', 'i', 'p', 't', '"', '>' };
  ...
  public Page(final JspWriter out) {
    ...
    this.m_htmlEncoded = XMLCharTransformer.getInstance()
        .transform(this.m_wrapped);
  }
  ...
  public final ITextOutput getJSEncoded() {
    if (this.m_jsEncoded == null) {
      this.m_jsEncoded = JavaCharTransformer.getInstance()
          .transform(this.m_wrapped);
    }
  ...
  public final void fieldNameFromPrefixAndNameJS(final boolean inString,
      final String prefix, final boolean prefixIsString, final String name,
      final boolean nameIsString) throws IOException {
    final JspWriter out;
