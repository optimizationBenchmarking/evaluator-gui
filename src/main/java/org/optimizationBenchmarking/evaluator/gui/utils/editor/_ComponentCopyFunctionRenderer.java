...
final class _ComponentCopyFunctionRenderer extends FunctionRenderer {
  ...
  public final void render(final Page page) throws IOException {
    ...
    out = page.getOut();
    out.write(
        "(globalPrefix,innerPrefix){div=document.getElementById(innerPrefix+'");//$NON-NLS-1$
    out.write(EditorModule.DIV_MAIN_SUFFIX);
    out.write(
        "');if(div!=null){temp=document.createElement('div');var copy=div.cloneNode(true);copy.style.display='block';temp.appendChild(copy);temp.innerHTML=temp.innerHTML.replace(new RegExp(innerPrefix,'g'),");//$NON-NLS-1$
