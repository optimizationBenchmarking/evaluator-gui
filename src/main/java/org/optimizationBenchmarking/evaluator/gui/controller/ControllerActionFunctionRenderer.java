package org.optimizationBenchmarking.evaluator.gui.controller;
...
public final class ControllerActionFunctionRenderer
    extends FunctionRenderer {
  ...
  public final void render(final Page page) throws IOException {
    ...
    out.write(
        "(prefix){var sel=document.getElementById(prefix+'Selection');if(sel!=null){var text='';var form=document.getElementById(prefix+'Form');var desc=document.getElementById(prefix+'Desc');if((form!=null)||(desc!=null)){switch(sel.value){"); //$NON-NLS-1$
    for (final SelectionAction action : this.m_actions) {
      action._jsRenderFormSwitchCase(out, encoded);
    }
    out.write(
        "default:{if(form!=null){form.action=window.location.pathname;form.method='get';form.target='_self';}}"); //$NON-NLS-1$
    out.write(
        "}if(desc!=null){if((text!=null)&&(text.length>0)){desc.innerHTML='"); //$NON-NLS-1$
