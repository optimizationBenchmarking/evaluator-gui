...
import org.optimizationBenchmarking.evaluator.gui.application.ApplicationInstanceBuilder;
import org.optimizationBenchmarking.evaluator.gui.utils.files.EChangeResult;
import org.optimizationBenchmarking.evaluator.gui.utils.files.FSElement;
  ...
  public final Path resolve(final Handle handle, final String relPath,
    ...
    if (FSElement.ROOT_PATH.equals(pre)) {
      ...
      if ((path == null) || (caught != null)) {
        handle.failure(
            "Failed to convert path name '" + relPath + //$NON-NLS-1$
                "' to a path.", //$NON-NLS-1$
            caught);
        return null;
      }
