...
public final class SelButtonFunctionRenderer extends FunctionRenderer {
  ...
  public final void render(final Page page) throws IOException {
    ...
    out = page.getOut();
    out.write(
        "(formId,value){var form=document.getElementById(formId);if(form!=null){var inputs=form.getElementsByTagName('input');if(inputs!=null){for(i=inputs.length;(--i)>=0;){var input=inputs[i];if(input.type=='checkbox'){if(input.name=='");//$NON-NLS-1$
