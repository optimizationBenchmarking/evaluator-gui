...
import org.optimizationBenchmarking.evaluator.gui.utils.FunctionRenderer;
...
final class _AutoRefineFunctionRenderer extends FunctionRenderer {
  ...
  public final void render(final Page page) throws IOException {
    ...
    out = page.getOut();
    out.write(
        "(typeId,lowerId,upperId){var select=document.getElementById(typeId);switch(select.value){case 'byte':case 'short':case 'int':case 'long':{if(document.getElementById(lowerId+'");//$NON-NLS-1$
    out.write(EditorModule.BUTTON_ENABLE_SUFFIX);
    out.write(
        "').value){var lower=document.getElementById(lowerId).value;if(lower!=null){lower=String(lower);if(lower.length>0){lower=parseFloat(lower);if(!(isNaN(lower))){if(document.getElementById(upperId+'");//$NON-NLS-1$
    out.write(EditorModule.BUTTON_ENABLE_SUFFIX);
    out.write(
        "').value){var upper=document.getElementById(upperId).value;if(upper!=null){upper=String(upper);if(upper.length>0){upper=parseFloat(upper);if(!(isNaN(upper))){");//$NON-NLS-1$

    out.write(
        "if(lower>upper){alert('Error: lower bound (' + lower + ') is greater than upper bound (' + upper + ').');}else{");//$NON-NLS-1$
    out.write(
        "if(lower>=upper){alert('Warning: lower bound (' + lower + ') is equal to upper bound (' + upper + ').');}}");//$NON-NLS-1$
