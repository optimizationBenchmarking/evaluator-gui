package org.optimizationBenchmarking.evaluator.gui.modules.evaluation;
...
import org.optimizationBenchmarking.evaluator.EvaluatorModulesBuilder;
import org.optimizationBenchmarking.evaluator.evaluation.definition.data.EvaluationModuleEntry;
import org.optimizationBenchmarking.evaluator.evaluation.definition.data.EvaluationModuleEntryBuilder;
...
import org.optimizationBenchmarking.evaluator.evaluation.definition.io.EvaluationXMLOutput;
...
import org.optimizationBenchmarking.evaluator.gui.modules.config.ConfigIO;
import org.optimizationBenchmarking.evaluator.gui.utils.Page;
import org.optimizationBenchmarking.evaluator.gui.utils.editor.EditorModule;
import org.optimizationBenchmarking.evaluator.gui.utils.files.Loaded;
import org.optimizationBenchmarking.evaluator.io.impl.edi.EDI;
...
public final class EvaluationIO extends EditorModule<EvaluationModules> {
  ...
  protected final EvaluationModules loadFile(final Path file,
    ...
    try (
        final EvaluatorModulesBuilder builder = new EvaluatorModulesBuilder()) {
  ...
  public final void formPutEditorFields(final String prefix,
    ...
    for (final EvaluationModuleEntry me : data.getEntries()) {
    ...
    this.formPutComponentHead(name, //
  ...
  protected final void formPutButtons(final String prefix,
    ...
    descriptions = this.__getDescriptions(null);
    out.write(
        page.getFunction(_AddFunctionRenderer._getInstance(descriptions)));
    out.write('(');
    out.write('\'');
      ...
      if (strings != null) {
          ...
          if (clazz == null) {
            ...
            try (final ConfigurationBuilder cb = builder
              ...
              ConfigIO.INSTANCE.loadConfigurationFromRequest(cb,
                  modulePrefix, descriptions.getJointParameters(), request,
                  handle);
            }
            break findConfig;
          }
        ...
        for (final String string : strings) {
          ...
          if ((clazz != null) && (modulePrefix != null)) {
            try (final EvaluationModuleEntryBuilder meb = builder
                .addModule()) {
              meb.setModule(clazz);

              definition = DefinitionXMLInput.getInstance()
                  .forClass(meb.getModule().getClass(), handle);

              try (
                  final ConfigurationBuilder cb = meb.setConfiguration()) {
                ConfigIO.INSTANCE.loadConfigurationFromRequest(cb,
  ...
  protected final void storeToFile(final EvaluationModules data,
    ...
    EvaluationXMLOutput.getInstance().use().setLogger(handle).setPath(file)
