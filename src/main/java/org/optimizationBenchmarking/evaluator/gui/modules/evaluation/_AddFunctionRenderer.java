...
final class _AddFunctionRenderer extends FunctionRenderer {
  ...
  public final void render(final Page page) throws IOException {
    ...
    out.write(
        "(prefix,button){var form=button;while((form!=null)&&(form.tagName.toUpperCase()!='FORM')){form=form.parentNode;}if(form!=null){var sel=document.getElementById(");//$NON-NLS-1$
    ...
    out.write(
        "default:{return;}}var dummy=document.createElement('form');dummy.innerHTML=text;var insert=dummy.firstChild;sel.parentNode.insertBefore(insert,sel);dummy.innerHTML='<input type=\"hidden\" name=\"");//$NON-NLS-1$
    out.write(ControllerUtils.INPUT_SUBMIT);
    out.write("\" value=\"");//$NON-NLS-1$
    out.write(ControllerUtils.PARAMETER_SAVE);
    out.write(
        "\"/>';insert=dummy.firstChild;sel.parentNode.insertBefore(insert,sel);form.submit();}}}");//$NON-NLS-1$
  }
}
