package org.optimizationBenchmarking.evaluator.gui.modules.instances;
...
public final class InstancesIO extends EditorModule<IInstanceSet> {
  ...
  public final void formPutEditorFields(final String prefix,
    ...
    for (final IInstance inst : new IterablePlusOne<>(data.getData(),
        null)) {
      ...
      if (inst != null) {
        lbloop: for (final IDimension dim : data.getOwner().getDimensions()
            .getData()) {
  ...
  protected final void formPutButtons(final String prefix,
      final Loaded<IInstanceSet> data, final Page page)
          throws IOException {
