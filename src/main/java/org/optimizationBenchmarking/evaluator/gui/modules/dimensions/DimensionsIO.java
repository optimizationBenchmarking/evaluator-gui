...
import org.optimizationBenchmarking.evaluator.data.impl.partial.PartialExperimentSetBuilder;
...
import org.optimizationBenchmarking.evaluator.io.impl.edi.FlatEDIInput;
...
public final class DimensionsIO extends EditorModule<IDimensionSet> {
  ...
  public final void formPutEditorFields(final String prefix,
    ...
    for (final IDimension dim : new IterablePlusOne<>(data.getData(),
        null)) {
      ...
      this.formPutSelection(typeId, //
          ...
          (page.getFunction(_TypeChangeFunctionRenderer.INSTANCE)
              + funcParams),
          page);
      ...
      this.formTableSpacer(page);
      this.formTableFieldRowBegin(lowerId, DimensionsIO.DIMENSION_MIN_NAME,
          true, page);
      parser = ((dim != null) ? dim.getParser()
          : LooseLongParser.INSTANCE);
      switch (type) {
        case FLOAT:
        case DOUBLE: {
          dbound = parser.getLowerBoundDouble();
          enabled = ((dbound > Double.NEGATIVE_INFINITY)
              && (dbound == dbound));
          this.formPutFloat(lowerId,
              enabled ? Double.valueOf(dbound) : null, page);
          break;
        }
        default: {
      ...
      this.formTableSpacer(page);
      this.formTableFieldRowBegin(upperId, DimensionsIO.DIMENSION_MAX_NAME,
          true, page);
      switch (type) {
        case FLOAT:
        case DOUBLE: {
          dbound = parser.getUpperBoundDouble();
          enabled = ((dbound < Double.POSITIVE_INFINITY)
              && (dbound == dbound));
          this.formPutFloat(upperId,
              enabled ? Double.valueOf(dbound) : null, page);
          break;
        }
        default: {
    ...
    if (strings != null) {
      for (final String dprefix : strings) {
        ...
        builder.dimensionSetDirection(//
            DimensionDirectionParser.INSTANCE
                .parseString(request.getParameter(//
                    Page.fieldNameFromPrefixAndName(dprefix,
                        DimensionsIO.DIMENSION_DIRECTION))));

        type = PrimitiveTypeParser.INSTANCE
            .parseString(request.getParameter(//
                Page.fieldNameFromPrefixAndName(dprefix,
                    DimensionsIO.DIMENSION_DATA_TYPE_ID)));
        ...
        try {
          id = Page.fieldNameFromPrefixAndName(dprefix,
              DimensionsIO.DIMENSION_MIN_ID);
          value = request
              .getParameter(id + EditorModule.BUTTON_ENABLE_SUFFIX);
          if (value != null) {
            ...
            builder.dimensionSetParser(//
                ((lower != null) || (upper != null)) ? //
                    new BoundedLooseByteParser(//
                        ((lower != null) ? lower.byteValue()
                            : Byte.MIN_VALUE), //
                        ((upper != null) ? upper.byteValue()
                            : Byte.MAX_VALUE))//
            ...
            builder.dimensionSetParser(//
                ((lower != null) || (upper != null)) ? //
                    new BoundedLooseLongParser(//
                        ((lower != null) ? lower.longValue()
                            : Long.MIN_VALUE), //
                        ((upper != null) ? upper.longValue()
                            : Long.MAX_VALUE))//
                    : LooseLongParser.INSTANCE);
            break;
          }
