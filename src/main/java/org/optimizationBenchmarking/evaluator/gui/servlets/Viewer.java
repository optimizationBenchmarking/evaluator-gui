package org.optimizationBenchmarking.evaluator.gui.servlets;
...
import org.optimizationBenchmarking.evaluator.gui.utils.Header;
...
public final class Viewer extends HttpServlet {
   ...
   *         otherwise
   */
  private static final boolean __is(final String mime,
      final String[] types) {
    if (mime == null) {
      return false;
    }
  ...
  private static final String __resolve(final String base,
    ...
    try {
      ...
      if (!(currentURI.isAbsolute())) {
        ...
        currentURL = (base
            + currentURL.substring((currentURL.charAt(0) == '/') ? 1 : 0));
          ...
          if (path != null) {
            try {
              if (Files.isRegularFile(path, LinkOption.NOFOLLOW_LINKS)) {
                type = FileTypeRegistry.getInstance()
